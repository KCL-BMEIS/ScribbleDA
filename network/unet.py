...
class UNet2D5(nn.Module):
    ...
    def __init__(
        ...
        for d in range(num_pool):
            ...
            # First block is modality-specific
            if d==0:
                self.conv_block_target = ConvNormNonlinBlock(input_features, output_features,
                                                                self.conv_op, self.conv_kwargs, self.norm_op,
                                                                self.norm_op_kwargs, self.nonlin, self.nonlin_kwargs)
    ...
    def forward(self, x, domain):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            if domain=='target' and d==0:
                x = self.conv_block_target(x)
            else:
